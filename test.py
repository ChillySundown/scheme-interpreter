...
def test_syntax_errors_2():
    ...
    # big valid tokenize / parse inputs that should have no syntax errors!
    for fname in ['ndmines', 'sudoku']:
        with open(os.path.join(TEST_DIRECTORY, 'test_files', f'{fname}.scm'), 'r') as f:
            text = f.read()

        with open(os.path.join(TEST_DIRECTORY, 'test_outputs', f'{fname}_tokenize.txt'), 'r') as f:
            expected_tokens = eval(f.read())

        with open(os.path.join(TEST_DIRECTORY, 'test_outputs', f'{fname}_parse.txt'), 'r') as f:
            expected_parse = eval(f.read())

        assert lab.tokenize(text) == expected_tokens
        assert lab.parse(lab.tokenize(text)) == expected_parse

# # BOOLEAN BUILT_INS
# def test_equal():
#     equal_args_1 = 3, 3, 3, 3, 3
#     assert lab.builtin_equal(equal_args_1) == True
#     equal_args_2 = 4, 4, 4, 4, 4, 8, 4, 4, 4, 4
#     assert lab.builtin_equal(equal_args_2) == False
...
def test_lists():
    do_raw_continued_evaluations(95)
